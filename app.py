...
# 📍 祇園用のAPI URL
API_URL = "https://mute-imari-1089.catfood.jp/gion/api.php"
...
def recalc_route_for_driver(drv_name, trigger_line_notify=False, manual_order=False):
    ...
    db_f = get_db_data()
    casts_f = db_f.get("casts", [])
    drvs_f = db_f.get("drivers", [])
    atts_f = db_f.get("attendance", [])
    sets_f = db_f.get("settings", {})
    # 📍 祇園用の住所
    store_addr_f = str(sets_f.get("store_address", "岡山県岡山市北区田町2丁目11-15"))
...
if current_page == "cast_mypage":
    ...
    line_uid = my_c.get("line_user_id", "")
...
if current_page == "staff_portal":
    ...
    # 📍 祇園用の住所
    store_addr = str(sets.get("store_address", "岡山県岡山市北区田町2丁目11-15"))
    ...
    if not is_adm:
                    ...
                    if st.button("変更を反映", type="primary", use_container_width=True):
                        ...
                        if len(my_assigned) > n_cap:
                            ...
                            if updates:
                                post_api({"action": "update_manual_dispatch", "updates": updates})
                                time.sleep(0.5)
                                recalc_route_for_driver(staff_n, trigger_line_notify=True, manual_order=False)
                                for u in updates:
        ...
        if my_early:
            ...
            if early_path:
                ...
                # 🛡️ マップURLの正規化
                map_url = f"https://maps.google.com/maps?saddr={org_enc}&daddr={d_enc}&waypoints={wp_enc}&dirflg=d"
        ...
        if my_tasks:
            ...
            list_html_head = f'<div style="background:#444; color:white; padding:10px; font-weight:bold; border-radius:5px 5px 0 0;">🚕 通常便ルート</div><div style="background:#ffffff; border:1px solid #ccc; border-top:none; padding:10px; border-radius:0 0 5px 5px; margin-bottom:20px; box-shadow: 0 2px 5px rgba(0,0,0,0.05);">'
            
            if is_return_time:
                list_html = list_html_head + f'<div style="background:#e3f2fd; border:2px solid #2196f3; padding:8px; border-radius:5px; margin-bottom:15px;"><div style="color:#1565c0; font-weight:bold; margin-bottom:5px;">🌙 帰り班 (自動編成)</div>'
                return_tasks = []
                ...
                if return_full_path:
                    ...
                    # 🛡️ マップURLの正規化
                    map_url = f"https://maps.google.com/maps?saddr={org_enc}&daddr={dest_enc}&waypoints={wp_enc}&dirflg=d"
...
if current_page == "staff_portal" and st.session_state.is_admin:
    tabs_list_admin = ["① 配車リスト", "② キャスト送迎", "③ キャスト登録", "④ STAFF設定", "⚙️ 管理設定"]
    if "admin_main_tabs" not in st.session_state: st.session_state.admin_main_tabs = "① 配車リスト"
    selected_tab = st.radio("メニュー", tabs_list_admin, key="admin_main_tabs", horizontal=True, label_visibility="collapsed")
    ...
    if selected_tab == "① 配車リスト":
        ...
        seen_cids_disp = set()
        for row in atts:
            if row["target_date"] == disp_date and row["status"] in ["出勤", "自走"]:
                cid_str = str(row["cast_id"])
        ...
        with st.expander("🌅 早便設定（一括追加ツール）", expanded=False):
            ...
            if st.button("➕ このキャストを早便リストに追加"):
                if selected_c != "-- 選択 --":
                    ...
                    st.session_state.early_form_key = fk + 1
                    st.rerun()
        ...
        # ===== ここから追加：確定ルートの一括LINE送信機能 =====
        if st.button("🚀 確定した送迎時間を全員に一括でLINE送信する", type="primary", use_container_width=True):
                        ...
                        if row["target_date"] == "当日" and row["status"] in ["出勤", "自走"]:
                            ...
                            if drv and drv != "未定" and pt and pt != "未定":
                                ...
                                if line_uid:
                                    ...
                                    success_count += 1
                                    time.sleep(0.1) # LINE側の制限に引っかからないための待機時間
        ...
        with st.form("adm_form"):
            ...
            # 📍 祇園用の住所
            s_addr = sets.get("store_address", "岡山県岡山市北区田町2丁目11-15") if isinstance(sets, dict) else "岡山県岡山市北区田町2丁目11-15"
        ...
        if st.button("🔄 本日の送迎データをリセットする", type="primary", use_container_width=True):
            reset_records = []
            for c in casts:
                reset_records.append({"cast_id": c["cast_id"], "cast_name": c["name"], "area": c.get("area", "他"), "status": "未定", "memo": "", "target_date": "当日"})
            if reset_records:
                res = post_api({"action": "save_attendance", "records": reset_records})
                if res.get("status") == "success":
                    clear_cache()
